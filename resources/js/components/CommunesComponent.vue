<template>
  <div class="container mt-4">
    <div class="card shadow-sm rounded-4">
      <div class="card-body">
        <div class="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-3">
          <h4 class="mb-0">{{ t("commune.title") }}</h4>
        ...
        <div class="table-responsive">
          <table class="table table-hover align-middle">
            <thead class="table-bg">
              <tr>
                <th width="80">{{ t("commune.tableId") }}</th>
                <th>{{ t("commune.province") }}</th>
                <th>{{ t("commune.district") }}</th>
                <th>{{ t("commune.communeNameEn") }}</th>
                <th>{{ t("commune.communeNameKh") }}</th>
                <th>{{ t("commune.communeCode") }}</th>
                <th>{{ t("commune.latitude") }}</th>
                <th>{{ t("commune.longitude") }}</th>
                <th width="180" class="text-end">{{ t("commune.action") }}</th>
              </tr>
            </thead>

            <tbody>
              <tr v-for="(c, index) in communes" :key="c.id || index">
                <td>{{ rowNumber(index) }}</td>
                <td>{{ c.province?.province_name_en || "-" }}</td>
                <td>{{ c.district?.district_name_en || "-" }}</td>
                <td>{{ c.commune_name_en }}</td>
                <td>{{ c.commune_name_kh }}</td>
                <td>{{ c.commune_code }}</td>
                <td>{{ c.latitude || "-" }}</td>
                <td>{{ c.longitude || "-" }}</td>
                <td class="text-end">
                  <button class="btn btn-sm btn-primary me-2" @click="openEditModal(c)">
                    <i class="fa-solid fa-pen-to-square"></i>
                    {{ t("commune.edit") }}
                  </button>
                  <button class="btn btn-sm btn-danger" @click="askDelete(c)">
              ...
              <tr v-if="!pageLoading && communes.length === 0">
    ...
    <!-- Create / Edit Modal -->
    <div class="modal fade" id="communeModal" tabindex="-1" aria-hidden="true" ref="communeModalEl">
      <div class="modal-dialog modal-dialog-centered modal-xl">
        <div class="modal-content rounded-4 border-0 shadow modal-gradient">
          ...
          <div class="modal-body pt-0 px-4 px-lg-5 pb-4">
            <form @submit.prevent="editMode ? updateCommune() : createCommune()">
              <div class="row g-4">
                <!-- Left Form -->
                <div class="col-lg-6">
                  <div class="form-panel p-4 rounded-4 shadow-sm">
                    <div class="row g-3">
                      <div class="col-md-12">
                        <label class="form-label custom-label">{{ t("commune.selectProvince") }}</label>
                        <select
                          ...
                          required
                        >
                          <option value="">{{ t("commune.selectProvinceOption") }}</option>
                          <option v-for="p in provinces" :key="p.id || p.province_code" :value="p.province_code">
                      ...
                      <div class="col-md-12">
                        <label class="form-label custom-label">{{ t("commune.selectDistrict") }}</label>
                        <select
                          class="form-select custom-input"
                          ...
                          required
                        >
                          <option value="">{{ t("commune.selectDistrictOption") }}</option>
                          <option
                      ...
                      <div class="col-md-12">
                        <label class="form-label custom-label">{{ t("commune.communeNameEnLabel") }}</label>
                        <input
                          type="text"
                          class="form-control custom-input"
                          v-model.trim="form.commune_name_en"
                          :placeholder="t('commune.communeNameEnPlaceholder')"
                          required
                        />
                      </div>

                      <div class="col-md-12">
                        <label class="form-label custom-label">{{ t("commune.communeNameKhLabel") }}</label>
                        <input
                          type="text"
                          class="form-control custom-input"
                          v-model.trim="form.commune_name_kh"
                          :placeholder="t('commune.communeNameKhPlaceholder')"
                          required
                        />
                      </div>

                      <div class="col-md-12">
                        <label class="form-label custom-label">{{ t("commune.communeCodeLabel") }}</label>
                        <input
                          type="text"
                          class="form-control custom-input bg-secondary-subtle"
                          v-model="form.commune_code"
                          :placeholder="t('commune.communeCodePlaceholder')"
                          readonly
                        />
                      </div>

                      <div class="col-md-6">
                        <label class="form-label custom-label">{{ t("commune.latitudeLabel") }}</label>
                        <input
                          type="number"
                          step="any"
                          class="form-control custom-input"
                          v-model="form.latitude"
                          :placeholder="t('commune.latitudePlaceholder')"
                          @input="updateMarkerFromInputs"
                        />
                      </div>

                      <div class="col-md-6">
                        <label class="form-label custom-label">{{ t("commune.longitudeLabel") }}</label>
                        <input
                          type="number"
                          step="any"
                          class="form-control custom-input"
                          v-model="form.longitude"
                          :placeholder="t('commune.longitudePlaceholder')"
                          @input="updateMarkerFromInputs"
                        />
                      </div>
                ...
                <div class="col-lg-6">
                  <div class="map-panel p-4 rounded-4 shadow-sm">
                    <div class="section-title">
                      ...
                      <label class="form-label custom-label mb-1">{{ t("commune.mapTitle") }}</label>
                    </div>
                    <div id="commune-map" class="map-box rounded-4"></div>
              ...
              <div class="modal-footer border-0 px-0 mt-4 justify-content-end gap-2">
                ...
                >
                  ...
                  {{ submitLoading ? t("commune.saving") : t("commune.save") }}
                </button>
                ...
                >
                  <i class="fa-solid fa-check me-1"></i>
                  {{ submitLoading ? t("commune.updating") : t("commune.update") }}
                </button>
              </div>
            </form>
          </div>
      ...
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content rounded-4 border-0 shadow">
          <div class="modal-body p-4 text-center">
            ...
            <h5 class="mb-2">{{ t("commune.confirmDeleteTitle") }}</h5>
            <p class="text-muted mb-0">{{ t("commune.confirmDeleteMessage") }}</p>
...
export default {
  ...
  async mounted() {
    ...
    this.communeModal = new Modal(this.$refs.communeModalEl, {
      backdrop: "static",
      keyboard: false,
    })
    ...
    await this.loadDistrictsForSelect()
    await this.loadCommunes(1)

    try {
      await this.loadGoogleMapsScript()
    } catch (err) {
      this.showAlert("error", this.t("commune.mapError"), this.t("commune.mapErrorMessage"))
    } finally {
  ...
  methods: {
    ...
    async loadProvinces() {
      try {
        ...
        this.showAlert("error", this.t("commune.loadFailed"), this.t("commune.loadProvinceFailedMessage"))
      }
    },

    async loadDistrictsForSelect() {
      try {
        ...
        console.log(err.response?.data || err.message)
        this.showAlert("error", this.t("commune.loadFailed"), this.t("commune.loadDistrictFailedMessage"))
      }
    },

    async loadCommunes(page = this.pagination.current_page) {
      this.pageLoading = true
      ...
      try {
        ...
        console.log(err.response?.data || err.message)
        this.communes = []
        this.showAlert("error", this.t("commune.loadFailed"), this.t("commune.loadCommuneFailedMessage"))
      } finally {
        this.pageLoading = false
      }
    },
    ...
    goToPrevPage() {
      if (this.pagination.current_page > 1) {
        this.loadCommunes(this.pagination.current_page - 1)
      }
    },
    ...
    goToLastPage() {
      if (this.pagination.current_page < this.pagination.last_page) {
        this.loadCommunes(this.pagination.last_page)
      }
    },
    ...
    getCurrentLocation() {
      if (!navigator.geolocation) {
        this.showAlert(
          "error",
          this.t("commune.geolocationError"),
          this.t("commune.geolocationNotSupported")
        )
        return
      }
      ...
      navigator.geolocation.getCurrentPosition(
        ...
        (error) => {
          ...
          let message = this.t("commune.unableToGetLocation")
          if (error.code === 1) message = this.t("commune.permissionDenied")
          if (error.code === 2) message = this.t("commune.locationUnavailable")
          if (error.code === 3) message = this.t("commune.locationTimeout")

          this.showAlert("error", this.t("commune.locationError"), message)
        },
        {
    ...
    async onDistrictChange() {
      ...
      try {
        const res = await axios.get(`/communes/next-code/${this.form.district_code}`)
        this.form.commune_code = res.data.commune_code || ""
      } catch (err) {
        console.log(err.response?.data || err.message)
        this.showAlert("error", this.t("commune.codeError"), this.t("commune.codeErrorMessage"))
      }
    },
    ...
    loadGoogleMapsScript() {
      return new Promise((resolve, reject) => {
        ...
        script.src = "https://maps.googleapis.com/maps/api/js?key=AIzaSyCs8T5do-m_YasDGHU0bcti_e15di4G8Hk"
        script.async = true
        script.defer = true
    ...
    initMap(lat = null, lng = null) {
      const mapEl = document.getElementById("commune-map")
    ...
    openEditModal(commune) {
      this.modalLoading = true
      this.editMode = true
      this.editId = commune.id

      this.form.province_code = commune.province_code || ""
      this.filteredDistricts = this.districts.filter(
        (d) => String(d.province_code) === String(this.form.province_code)
      )

      this.form.district_code = commune.district_code || ""
      this.form.commune_name_en = commune.commune_name_en || ""
      this.form.commune_name_kh = commune.commune_name_kh || ""
      this.form.commune_code = commune.commune_code || ""
      this.form.latitude = commune.latitude || ""
      this.form.longitude = commune.longitude || ""
    ...
    async createCommune() {
      if (
        ...
        !this.form.district_code ||
        !this.form.commune_name_en ||
        !this.form.commune_name_kh
      ) {
        this.showAlert("warning", this.t("commune.validationError"), this.t("commune.fillRequired"))
        return
      }
      ...
      try {
        await axios.post("/communes", {
          province_code: this.form.province_code,
          district_code: this.form.district_code,
          commune_name_en: this.form.commune_name_en,
          commune_name_kh: this.form.commune_name_kh,
        ...
        this.resetForm()
        this.showAlert("success", this.t("commune.success"), this.t("commune.createdSuccess"))
      } catch (err) {
        if (err.response?.status === 422) {
          const errors = err.response.data.errors
          const firstMsg =
            Object.values(errors || {})?.[0]?.[0] || this.t("commune.validationMessage")
          this.showAlert("warning", this.t("commune.validationError"), firstMsg)
          return
        }
        ...
        this.showAlert(
          "error",
          this.t("commune.createFailed"),
          err.response?.data?.message || err.message || this.t("commune.createFailedMessage")
        )
      } finally {
    ...
    async updateCommune() {
      ...
      try {
        await axios.put(`/communes/${this.editId}`, {
          province_code: this.form.province_code,
          district_code: this.form.district_code,
          commune_name_en: this.form.commune_name_en,
          commune_name_kh: this.form.commune_name_kh,
        ...
        await this.loadCommunes(this.pagination.current_page)
        this.communeModal.hide()
        this.resetForm()
        this.showAlert("success", this.t("commune.success"), this.t("commune.updatedSuccess"))
      } catch (err) {
        if (err.response?.status === 422) {
          const errors = err.response.data.errors
          const firstMsg =
        ...
        this.showAlert(
          "error",
          this.t("commune.updateFailed"),
          err.response?.data?.message || err.message || this.t("commune.updateFailedMessage")
        )
      } finally {
        this.submitLoading = false
      }
    ...
    async confirmDelete() {
      ...
      this.deleting = true
      try {
        await axios.delete(`/communes/${this.deleteTarget.id}`)

        this.confirmDeleteModal?.hide()

        let targetPage = this.pagination.current_page
        const isLastItemOnPage = this.communes.length === 1 && targetPage > 1

        if (isLastItemOnPage) {
          targetPage -= 1
        }

        await this.loadCommunes(targetPage)

        this.showAlert("success", this.t("commune.deletedTitle"), this.t("commune.deletedSuccess"))
      } catch (err) {
        console.log(err.response?.data || err.message)
        this.showAlert(
          "error",
          this.t("commune.deleteFailed"),
          err.response?.data?.message || err.message || this.t("commune.deleteFailedMessage")
        )
      } finally {
        this.deleting = false
