<template>
  <div class="container-fluid mt-4">
      ...
      <div class="card-body">
        <div class="d-flex justify-content-between mb-3">
          ...
          <button class="btn btn-success" @click="openCreateModal">
            <i class="fa-solid fa-circle-plus"></i> {{ t('od.createButton') }}
          </button>
        </div>
          ...
          <table class="table table-hover align-middle">
            <thead class="table-bg">
              <tr>
                ...
                <th>{{ t('od.odNameKh') }}</th>
                <th>{{ t('od.odCode') }}</th>
                <th>{{ t('od.directorName') }}</th>
                <th>{{ t('od.phone') }}</th>
                <th>{{ t('od.latitude') }}</th>
                <th>{{ t('od.longitude') }}</th>
                <th>{{ t('od.image') }}</th>
                <th width="180" class="text-end">{{ t('od.action') }}</th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="(o, index) in ods" :key="o.id || index">
                <td>{{ index + 1 }}</td>
                <td>{{ o.province?.province_name_en || "-" }}</td>
                <td>{{ o.district?.district_name_en || "-" }}</td>
                <td>{{ o.commune?.commune_name_en || "-" }}</td>
                <td>{{ o.od_name_en }}</td>
                <td>{{ o.od_name_kh }}</td>
                <td>{{ o.od_code }}</td>
                <td>{{ o.name_director || "-" }}</td>
                <td>{{ o.phone || "-" }}</td>
                <td>{{ o.latitude || "-" }}</td>
                <td>{{ o.longitude || "-" }}</td>
                <td>
                  <img v-if="o.image" :src="`/storage/${o.image}`" :alt="t('od.imageAlt')" class="table-image" />
                  <span v-else>-</span>
                </td>
                <td class="text-end">
                  <button class="btn btn-sm btn-primary me-2" @click="openEditModal(o)">
                    <i class="fa-solid fa-pen-to-square"></i> {{ t('od.edit') }}
                  </button>
                  <button class="btn btn-sm btn-danger" @click="askDelete(o)">
                    <i class="fa-solid fa-trash"></i> {{ t('od.delete') }}
                  </button>
                </td>
              </tr>

              <tr v-if="ods.length === 0">
                <td colspan="13" class="text-center text-muted py-4">{{ t('od.noData') }}</td>
              </tr>
            </tbody>
          </table>
        </div>
    ...
    <div class="modal fade" id="odModal" tabindex="-1" aria-hidden="true" ref="odModalEl">
        ...
        <div class="modal-content rounded-4 border-0 shadow modal-gradient">
          <div class="modal-header border-0 flex-column text-center position-relative pb-2">
            ...
            <div v-if="!editMode" class="w-100">
              ...
              <h5 class="modal-title title-border pb-2">{{ t('od.createTitle') }}</h5>
              <p class="text-muted small mb-0">{{ t('od.createSubtitle') }}</p>
            </div>

            <div v-else class="w-100">
              <h5 class="modal-title title-border pb-2 mt-3">{{ t('od.editTitle') }}</h5>
              <p class="text-muted small mb-0">{{ t('od.editSubtitle') }}</p>
            </div>
          </div>

          <div class="modal-body pt-3 px-4 px-md-5">
            <form @submit.prevent="editMode ? updateOD() : createOD()">
              <div class="form-section">
                ...
                <div class="row g-3">
                  <div class="col-md-6">
                    <label class="form-label">{{ t('od.selectProvince') }}</label>
                    <select class="form-select custom-input" v-model="form.province_code" @change="onProvinceChange"
                      required>
                      <option value="">{{ t('od.selectProvinceOption') }}</option>
                  ...
                  <div class="col-md-6">
                    <label class="form-label">{{ t('od.selectDistrict') }}</label>
                    <select class="form-select custom-input" v-model="form.district_code" @change="onDistrictChange"
                      required>
                      <option value="">{{ t('od.selectDistrictOption') }}</option>
                  ...
                  <div class="col-md-6">
                    <label class="form-label">{{ t('od.selectCommune') }}</label>
                    <select class="form-select custom-input" v-model="form.commune_code" @change="onCommuneChange"
                      required>
                      <option value="">{{ t('od.selectCommuneOption') }}</option>
                  ...
                  <div class="col-md-6">
                    <label class="form-label">{{ t('od.odNameEnLabel') }}</label>
                    <input type="text" class="form-control custom-input" v-model.trim="form.od_name_en" required />
                  </div>

                  <div class="col-md-6">
                    <label class="form-label">{{ t('od.odNameKhLabel') }}</label>
                    <input type="text" class="form-control custom-input" v-model.trim="form.od_name_kh" required />
                  </div>

                  <div class="col-md-6">
                    <label class="form-label">{{ t('od.odCodeLabel') }}</label>
                    <input type="text" class="form-control custom-input bg-secondary-subtle" v-model.trim="form.od_code"
                      :placeholder="t('od.odCodePlaceholder')" readonly />
                  </div>

                  <div class="col-md-6">
                    <label class="form-label">{{ t('od.directorNameLabel') }}</label>
                    <input type="text" class="form-control custom-input" v-model.trim="form.name_director"
                      :placeholder="t('od.directorNamePlaceholder')" />
                  </div>

                  <div class="col-md-6">
                    <label class="form-label">{{ t('od.phoneLabel') }}</label>
                    <input type="text" class="form-control custom-input" v-model.trim="form.phone"
                      :placeholder="t('od.phonePlaceholder')" />
                  </div>

                  <div class="col-md-6">
                    <label class="form-label">{{ t('od.imageLabel') }}</label>
                    <input type="file" class="form-control custom-input-file" accept="image/*"
                      @change="handleImageUpload" />
                  </div>

                  <div class="col-md-6" v-if="imagePreview">
                    <label class="form-label">{{ t('od.previewLabel') }}</label>
                    <div v-if="imagePreview"></div>
                    <div class="preview-wrapper">
                      <img :src="imagePreview" :alt="t('od.previewAlt')" class="img-preview" />

                      <button type="button" class="remove-preview-btn" @click="removePreviewImage"
                        :title="t('od.removeImage')">
              ...
              <div class="form-section mt-4">
                ...
                <div id="od-map" class="map-box rounded-4 shadow-sm"></div>
              ...
              <div class="modal-footer border-0 px-0 mt-4">
                ...
                <button v-if="!editMode" type="submit" class="btn btn-success btn-action">
                  <i class="fa-solid fa-check me-1"></i> {{ t('od.save') }}
                </button>

                <button v-else type="submit" class="btn btn-primary btn-action">
                  <i class="fa-solid fa-check me-1"></i> {{ t('od.update') }}
                </button>
              </div>
            </form>
          </div>
      ...
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content rounded-4 border-0 shadow">
          <div class="modal-body p-4 text-center">
            ...
            <h5 class="mb-2">{{ t('od.confirmDeleteTitle') }}</h5>
            <p class="text-muted mb-0">{{ t('od.confirmDeleteMessage') }}</p>
    ...
    <div v-if="pageLoading || modalLoading || submitLoading" class="loading-overlay" role="dialog" aria-modal="true">
      <div class="loading-box">
        ...
        <div class="loading-text">{{ t('od.loading') }}</div>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  async mounted() {
    ...
    this.odModal = new Modal(this.$refs.odModalEl, {
      backdrop: "static",
      keyboard: false,
    })
    ...
    try {
      ...
      this.showAlert("error", this.$t("od.mapError"), this.$t("od.mapErrorMessage"))
    } finally {
  ...
  methods: {
    ...
    getCurrentLocation() {
      if (!navigator.geolocation) {
        this.showAlert(
          "error",
          this.$t("od.geolocationError"),
          this.$t("od.geolocationNotSupported")
        )
        return
      }
      ...
      navigator.geolocation.getCurrentPosition(
        ...
        (error) => {
          ...
          let message = this.$t("od.unableToGetLocation")
          if (error.code === 1) message = this.$t("od.permissionDenied")
          if (error.code === 2) message = this.$t("od.locationUnavailable")
          if (error.code === 3) message = this.$t("od.locationTimeout")

          this.showAlert("error", this.$t("od.locationError"), message)
        },
        {
    ...
    async loadProvinces() {
      try {
        ...
        this.showAlert(
          "error",
          this.$t("od.loadFailed"),
          this.$t("od.loadProvinceFailedMessage")
        )
      }
    },
    ...
    async loadDistricts() {
      try {
        ...
        this.showAlert(
          "error",
          this.$t("od.loadFailed"),
          this.$t("od.loadDistrictFailedMessage")
        )
      }
    },
    ...
    async loadCommunes() {
      try {
        ...
        this.showAlert(
          "error",
          this.$t("od.loadFailed"),
          this.$t("od.loadCommuneFailedMessage")
        )
      }
    },
    ...
    async onCommuneChange() {
      ...
      if (!this.form.commune_code) return

      try {
        const res = await axios.get(`/ods/next-code/${this.form.commune_code}`)
        this.form.od_code = res.data.od_code || ""
      } catch (err) {
        console.log(err.response?.data || err.message)
        this.showAlert(
          "error",
          this.$t("od.codeError"),
          this.$t("od.codeErrorMessage")
        )
      }
    },
    ...
    initMap(lat = null, lng = null) {
      const mapEl = document.getElementById("od-map")
    ...
    openEditModal(od) {
      ...
      this.form.province_code = od.province_code || ""
      this.filteredDistricts = this.districts.filter(
        (d) => String(d.province_code) === String(this.form.province_code)
      )

      this.form.district_code = od.district_code || ""
      this.filteredCommunes = this.communes.filter(
        (c) => String(c.district_code) === String(this.form.district_code)
      )

      this.form.commune_code = od.commune_code || ""
      this.form.od_name_en = od.od_name_en || ""
      this.form.od_name_kh = od.od_name_kh || ""
      this.form.od_code = od.od_code || ""
      this.form.name_director = od.name_director || ""
      this.form.phone = od.phone || ""
      this.form.image = null
      this.imagePreview = od.image ? `/storage/${od.image}` : ""
      this.form.latitude = od.latitude || ""
      this.form.longitude = od.longitude || ""
    ...
    async createOD() {
      if (
        ...
        !this.form.district_code ||
        !this.form.commune_code ||
        !this.form.od_name_en ||
        !this.form.od_name_kh ||
        !this.form.od_code
      ) {
        this.showAlert(
          "warning",
          this.$t("od.validationError"),
          this.$t("od.fillRequired")
        )
        return
      }
      ...
      try {
        ...
        formData.append("od_name_en", this.form.od_name_en)
        formData.append("od_name_kh", this.form.od_name_kh)
        ...
        if (err.response?.status === 422) {
          ...
          const firstMsg =
            ...
            this.$t("od.validationMessage")

          this.showAlert("warning", this.$t("od.validationError"), firstMsg)
          return
        }
        ...
        this.showAlert(
          "error",
          this.$t("od.createFailed"),
          err.response?.data?.message || err.message || this.$t("od.createFailedMessage")
        )
      } finally {
    ...
    async updateOD() {
      ...
      try {
        ...
        await axios.post(`/ods/${this.editId}`, formData, {
          headers: {
            "Content-Type": "multipart/form-data",
          },
        })
        ...
        this.showAlert(
          "error",
          this.$t("od.updateFailed"),
          err.response?.data?.message || err.message || this.$t("od.updateFailedMessage")
        )
      } finally {
        this.submitLoading = false
      }
    ...
    async confirmDelete() {
      ...
      this.deleting = true
      try {
        await axios.delete(`/ods/${this.deleteTarget.id}`)
        this.ods = this.ods.filter((o) => o.id !== this.deleteTarget.id)

        this.confirmDeleteModal?.hide()
        this.showAlert(
          "success",
          this.$t("od.deletedTitle"),
          this.$t("od.deletedSuccess")
        )
      } catch (err) {
        console.log(err.response?.data || err.message)
        this.showAlert(
          "error",
          this.$t("od.deleteFailed"),
          err.response?.data?.message || err.message || this.$t("od.deleteFailedMessage")
        )
      } finally {
        this.deleting = false
